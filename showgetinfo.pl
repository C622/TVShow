...
if ( $previous_episode_date_line eq "" ) {
	if ( $previous_episode_date_line = return_single ($previous_episode_date, "AT ", "\$") ) {
		$previous_episode_date_line = "Tonight at $previous_episode_date_line";
	}
}
...
unless ($previous_episode_date_line eq "") {
	unless ($previous_episode_date_line =~ m/^Tonight at/) {
